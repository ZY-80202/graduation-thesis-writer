...
def generate_diagrams(
    ...
    definitions = [
        ("architecture", "系统总体架构图", captions.figure(4, "系统总体架构图"), _architecture(project)),
        ("function_structure", "系统功能结构图", captions.figure(4, "系统功能结构图"), _function_structure(project)),
        ("business_flow", "系统业务流程图", captions.figure(3, "系统业务流程图"), _business_flow(project)),
        ("user_flow", "用户操作流程图", captions.figure(5, "用户操作流程图"), _user_flow(project)),
        ("admin_flow", "管理员操作流程图", captions.figure(5, "管理员操作流程图"), _admin_flow(project)),
        ("er", "数据库 ER 图", captions.figure(4, "数据库 ER 图"), _er_diagram(project)),
        ("frontend_backend", "前后端交互流程图", captions.figure(4, "前后端交互流程图"), _frontend_backend(project)),
        ("test_flow", "系统测试流程图", captions.figure(6, "系统测试流程图"), _test_flow(project)),
    ]
    artifacts: List[DiagramArtifact] = []
    for key, title, caption, data in definitions:
        boxes, lines, mermaid = data
        base = safe_slug(key)
        png_path = out_dir / f"{base}.png"
        svg_path = out_dir / f"{base}.svg"
        mmd_path = out_dir / f"{base}.mmd"
        _draw_png(png_path, title, boxes, lines)
        _write_svg(svg_path, title, boxes, lines)
        write_text(mmd_path, mermaid)
        artifacts.append(DiagramArtifact(key=key, title=title, caption=caption, png_path=str(png_path), svg_path=str(svg_path), mermaid_path=str(mmd_path)))
...
def _user_flow(project: ProjectProfile) -> Tuple[List[Box], List[Line], str]:
    modules = [item for item in project.function_modules if "后台" not in item and "管理" not in item][:4]
    steps = ["进入系统", "登录/注册", *(modules or ["使用核心功能"]), "查看处理结果"]
    return _horizontal_flow(steps[:6], "flowchart LR")


def _admin_flow(project: ProjectProfile) -> Tuple[List[Box], List[Line], str]:
    modules = [item for item in project.function_modules if "管理" in item or "后台" in item][:4]
    steps = ["管理员登录", "进入后台", *(modules or ["维护基础数据", "审核业务信息"]), "查看统计结果"]
    return _horizontal_flow(steps[:6], "flowchart LR")
...
def _draw_png(path: Path, title: str, boxes: Iterable[Box], lines: Iterable[Line]) -> None:
...
def _write_svg(path: Path, title: str, boxes: Iterable[Box], lines: Iterable[Line]) -> None:
    parts = [
...
def _draw_wrapped_text(draw: ImageDraw.ImageDraw, label: str, box: Tuple[int, int, int, int], font: ImageFont.ImageFont) -> None:
    x1, y1, x2, y2 = box
    rows: List[str] = []
    for raw in label.splitlines():
        if len(raw) <= 10:
            ...
            rows.extend([raw[index : index + 10] for index in range(0, len(raw), 10)])
    line_height = 22
